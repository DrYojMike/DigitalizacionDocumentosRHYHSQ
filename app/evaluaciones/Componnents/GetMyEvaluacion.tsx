...
import {FaUser,FaUserTie,FaCheckCircle,FaChartBar,} from "react-icons/fa";
import { EvaluacionCompleta } from "@/app/evaluaciones/types/GetMyEvaluacion";
...
export default function GetMyEvaluation({ idevageneral }: Props) {
      ...
      {data.areas.map((area) => (
        <div key={area.idArea} className="mb-10">
          ...
          {area.competencias.map((competencia) => (
            <div key={competencia.idCompetencia} className="bg-white rounded-xl shadow mb-6">
              ...
              <div className="p-5 space-y-4">
                {competencia.indicadores.map((indicador) => {
                  ...
                  return (
                    <div key={indicador.idIndicador} className="border rounded-xl p-5">
                      ...
                      <div className="grid md:grid-cols-3 gap-5">
                        ...
                        <div className={`${promedioStyle.bg} rounded-xl p-5 text-center`}>
                          ...
                          <span className={`inline-block mt-3 px-3 py-1 rounded-full text-sm font-semibold ${promedioStyle.bg} ${promedioStyle.text}`}>
                            {promedioStyle.label}
                          </span>
                        </div>
                      </div>
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        <div className="space-y-3">
          {data.compromisos.map((compromiso) => (
            <div key={compromiso.idCompromiso} className="border-l-4 border-red-600 bg-gray-50 p-4 rounded">
